...
def check_technician_eligibility(emp, service_name=None, prefetched_data=None, job=None):
    ...
    GT audit Update 14/Phase 3 fix: this wrapper used to drop the `job`
    argument entirely, which meant every manual caller of it silently
    skipped automatic_dispatch.py's Gate 3 fine-grained purchased-vehicle-
    class/capacity check (the check only runs when a job is supplied, and
    falls back to the coarse category-only check otherwise -- see that
    module's docstring). Automatic dispatch always passed `job=job_obj`
    directly to check_candidate_eligibility and was never affected; only
    callers going through THIS wrapper were exposed to the gap. Forwarding
    `job` here closes it without touching automatic_dispatch.py again.
    """
...
class WorkforceDispatchEligibleListView(APIView):
    ...
    def get(self, request):
        ...
        for emp in candidates:
            ...
            # GT audit Phase 3 fix: pass `job` through so this admin-facing
            # eligibility preview reflects the SAME purchased-vehicle-class
            # compatibility gate that automatic dispatch enforces, instead of
            # only the coarse category-level check. `job` is already resolved
            # above from job_id when supplied; None (no job_id given) is a
            # no-op for the fine-grained check, same as before this fix.
            is_eligible, reason, gate_results = check_candidate_eligibility(emp, service_name, job=job)
...
class WorkforceJobAcceptOfferView(APIView):
    ...
    def post(self, request, pk):
        ...
        with transaction.atomic():
            ...
            #
            # GT audit Phase 3 fix: pass `job=job_obj` through so manual
            # offer acceptance enforces the same purchased-vehicle-class/
            # capacity compatibility gate (automatic_dispatch.py Gate 3,
            # Update 14) as automatic dispatch. Before this fix,
            # check_technician_eligibility() silently dropped `job`, so a
            # technician could accept an offer with an incompatible vehicle
            # (e.g. a two-wheeler rider self-accepting a heavy-truck GT job)
            # even though the same vehicle would have been rejected by
            # automatic dispatch. This is a compatibility check on a
            # specific booking's already-computed requirement, not a new
            # business rule -- it does not change what automatic dispatch
            # allows or restrict any other service category.
            # GT audit Update 17: this ran only when there was NO offer, on
            # the assumption that an offer had already been vetted at
            # dispatch time. Eligibility is not a fact about the offer, it
            # is a fact about the technician RIGHT NOW: between dispatch and
            # acceptance a vehicle's insurance/permit/PUC can lapse, the
            # vehicle can be deactivated, or the only class-compatible
            # vehicle can be removed from the fleet. Accepting on a stale
            # vetting puts an uninsured or undersized vehicle on a live
            # booking, which is precisely what Gate 3 exists to prevent.
            #
            # Checking on every acceptance path is strictly additive: it
            # adds no new rule, it just stops an existing one from being
            # satisfied by a snapshot taken minutes earlier.
            # GT audit Update 18: compatibility is checked on EVERY
            # acceptance path; the full ten-gate engine only on the
            # unvetted one.
            #
            # An offer proves the platform vetted this technician at
            # dispatch time. It does not prove the vehicle is still valid
            # now: between offer and acceptance a vehicle's insurance,
            # permit or PUC can lapse, or the only class-compatible vehicle
            # can be deactivated -- and the customer would get a vehicle
            # that cannot legally or physically do the job they paid for.
            #
            # Re-running the WHOLE engine here was tried and rejected: it
            # carries unrelated operational gates (cash float ceiling,
            # workload) that would refuse an offer the platform itself
            # issued. So acceptance calls the one narrow compatibility
            # authority instead -- the same function automatic dispatch
            # calls from Gate 3, never a second copy of its rules. It
            # returns immediately for non-logistics categories and touches
            # no database for them.
            from workforce_api.services.automatic_dispatch import (
                check_purchased_vehicle_compatibility,
            )

            compatible, compat_reason = check_purchased_vehicle_compatibility(
                emp_obj, job_obj.service_category, job_obj
            )
            if not compatible and job_obj.issue_title:
                by_title, title_reason = check_purchased_vehicle_compatibility(
                    emp_obj, job_obj.issue_title, job_obj
                )
                if by_title:
                    compatible, compat_reason = True, ""
                else:
                    compat_reason = title_reason or compat_reason
            if not compatible:
                return Response(
                    {"error": f"Cannot accept offer: {compat_reason}",
                     "code": "INCOMPATIBLE_VEHICLE"},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            # No offer behind this acceptance means nothing vetted this
            # technician at all, so the full engine still applies there --
...
class WorkforceJobRejectOfferView(APIView):
    ...
    def post(self, request, pk):
        ...
        # GT audit Update 17: you can only decline something you were
        # actually given.
        #
        # Tenancy alone was the whole guard here, and for a platform job
        # (company_id None or 1) every solo and platform technician passes
        # tenancy. So any of them could POST reject-offer on a job they had
        # never been offered: the handler below fabricated a REJECTED offer
        # row for them, re-ran automatic dispatch, and could flip a live job
        # back to "unassigned" -- dispatch churn and a falsified offer
        # history, triggered by a caller with no relationship to the job.
        #
        # A genuine decline always has one of two shapes: an offer row for
        # this employee, or this employee being the currently assigned one
        # (declining an assignment made without an offer row). Anything else
        # is refused, and with the same 404 the "no such job" branch above
        # returns so ids stay non-enumerable.
        has_offer = WorkforceJobOffer.objects.filter(job=job, employee=emp).exists()
        if not has_offer and job.assigned_employee != emp:
            return Response({"error": "Job not found."}, status=status.HTTP_404_NOT_FOUND)
...
class WorkforceJobLiveTrackingView(APIView):
    ...
    def get(self, request, pk):
        ...
        # GT audit Update 17: identity here must come from something the
        # caller cannot choose for themselves.
        #
        # This previously also accepted a match between the booking's
        # customer_name (a free-text display name) or phone and the
        # caller's USERNAME. accounts.User.username is a free-form unique
        # CharField chosen at registration, so either match was fully
        # attacker-controllable: registering the username "Ravi Kumar", or
        # a username that is a victim's phone number, granted live tracking
        # -- the technician's real-time GPS, the customer's address and
        # phone -- for every booking carrying that name or number.
        #
        # The FK is the authoritative link. The phone fallback is kept for
        # bookings placed against a phone number before/without an account
        # link, but is now matched against User.phone / User.mobile_number,
        # which are unique, registration-validated identity fields, never
        # against the username. The display-name match is removed outright:
        # a name is not an identity.
        _caller_phones = {
            str(getattr(user, "phone", "") or "").strip(),
            str(getattr(user, "mobile_number", "") or "").strip(),
        } - {""}
        _job_phone = str(getattr(job, "phone", "") or "").strip()
        is_owner_customer = bool(
            user.is_authenticated and (
                job.customer == user
                or (_job_phone and _job_phone in _caller_phones)
            )
        )
...
class WorkforceJobLogisticsLegView(APIView):
    ...
    def get(self, request, pk):
        ...
        # GT audit Update 17: this GET had no ownership check whatsoever --
        # any approved technician, from any company, could read any job's
        # current leg and its FULL leg history (every state change and its
        # timestamp) by walking numeric ids. POST on this same endpoint has
        # always required both assignment and tenancy; the read is now held
        # to the identical standard, using the same two guards in the same
        # order, so the two verbs cannot drift apart again.
        emp = getattr(request.user, "employee_profile", None)
        if not emp or job.assigned_employee != emp:
            return Response({"error": "Unauthorized: Job is not assigned to you."}, status=status.HTTP_403_FORBIDDEN)
        if not is_employee_authorized_for_job(emp, job):
...
class WorkforceJobMessagesView(APIView):
    ...
    def get(self, request, pk):
        ...
        if not emp or job.assigned_employee != emp:
            return Response({"error": "Unauthorized: Job is not assigned to you."}, status=status.HTTP_403_FORBIDDEN)
        # GT audit Update 17: same reasoning the leg and stop endpoints in
        # this file already carry -- an assignment can outlive a technician
        # moving between companies, and this thread holds customer-written
        # content. Assignment is not tenancy; both are required.
        if not is_employee_authorized_for_job(emp, job):
            return Response(
                {"error": "Unauthorized access to job belonging to another company.",
                 "code": "CROSS_TENANT_FORBIDDEN"},
    ...
    def post(self, request, pk):
        ...
        if not emp or job.assigned_employee != emp:
            return Response({"error": "Unauthorized: Job is not assigned to you."}, status=status.HTTP_403_FORBIDDEN)
        # GT audit Update 17: see the GET above.
        if not is_employee_authorized_for_job(emp, job):
            return Response(
                {"error": "Unauthorized access to job belonging to another company.",
                 "code": "CROSS_TENANT_FORBIDDEN"},
