...
# GT audit Update 14: rank of the purchased ServiceTier.vehicle_class
# (Customer app, logistics/models.py's VehicleClass choices) on a single
# "bigger vehicle = higher rank" scale. Mirrored here rather than imported,
# same reason as LOGISTICS_SERVICE_CATEGORIES above: two separate Django
# projects sharing a database, not a codebase.
_TIER_VEHICLE_CLASS_RANK = {
    "two_wheeler": 1,
    "three_wheeler": 2,
    "pickup": 3,
    "truck": 4,
...
# Rank of this app's own Vehicle.VehicleType (workforce_api/models.py) on
# the SAME scale as _TIER_VEHICLE_CLASS_RANK above, so a candidate's actual
# vehicle can be compared against what the customer purchased, not merely
# against a coarse service_category bucket. "mini_truck" is ranked equal to
# "truck": that equivalence isn't invented for this fix -- it's the same
# equivalence the pre-existing coarse allowed_types set below already
# encoded (mini_truck was already accepted anywhere "truck" was).
# VehicleType.OTHER has no rank and is intentionally absent -- an
# unclassified vehicle can never satisfy a class-ranked requirement below
# (fails closed rather than guessing).
_EMPLOYEE_VEHICLE_TYPE_RANK = {
    "two_wheeler": 1,
    "three_wheeler": 2,
    "pickup": 3,
    "mini_truck": 4,
...
def employees_with_live_offers(exclude_job=None) -> set:
    """
    ...
    both try to offer them a job at the same moment.
    """
...
def check_purchased_vehicle_compatibility(emp, service_name, job=None):
    """
    GT audit Update 18 -- the ONE authority on whether a candidate can
    actually fulfil what a given booking purchased.

    Returns (ok, reason). `reason` is "" when ok.

    This is the logistics half of Gate 3, lifted out verbatim so it has
    exactly one definition. It answers only compatibility questions:

      * does this technician have an active vehicle at all,
      * are its insurance/permit/PUC current,
      * is its type in the coarse allow-list for this service category,
      * and does it meet the SPECIFIC purchased requirement -- the tier's
        vehicle_class for goods transport, or payload_kg for Packers &
        Movers -- read from the booking's immutable quote-time snapshot,
        never from today's mutable ServiceTier.

    It deliberately answers nothing else. check_candidate_eligibility calls
    it as part of the full ten-gate dispatch sweep; offer ACCEPTANCE calls
    it on its own, because at that point the platform has already issued
    the offer and re-running unrelated operational gates (an unsettled cash
    float, a workload ceiling) would reject an offer the platform itself
    created. Compatibility is the one thing that must hold no matter how
    the technician arrived at the job.

    Non-logistics service categories return (True, "") immediately and
    touch no database.
    """
    service_name_clean = (service_name or "").strip().lower()
    if service_name_clean not in LOGISTICS_SERVICE_CATEGORIES:
        return True, ""
    ...
    if not vehicles:
        ...
        return False, "Gate 3: No active vehicle on file for this logistics job."
    ...
    if not current_vehicles:
        ...
        return False, "Gate 3: Vehicle insurance, permit or PUC has expired."

    # Category-specific vehicle compatibility check (coarse). Kept as a
    # first, cheap filter -- it only proves "some kind of truck", not
    # that this specific vehicle meets the SPECIFIC ServiceTier the
    # customer purchased (a three_wheeler and a heavy_truck both pass
    # this alone under goods_transport_truck). The finer check below
    # closes that gap.
    if service_name_clean == "goods_transport_two_wheeler":
        ...
        if not any(str(getattr(v, "vehicle_type", "")).strip().lower() in allowed_types for v in current_vehicles):
            return False, "Gate 3: Two-wheeler vehicle required for two-wheeler logistics."
    elif service_name_clean == "goods_transport_truck":
        allowed_types = {"truck", "mini_truck", "three_wheeler", "tempo", "commercial", "pickup", "canter"}
        if not any(str(getattr(v, "vehicle_type", "")).strip().lower() in allowed_types for v in current_vehicles):
            return False, "Gate 3: Truck or commercial vehicle required for truck logistics."
    elif service_name_clean == "packers_movers":
        allowed_types = {"truck", "mini_truck", "canter", "tempo", "commercial", "relocation_truck"}
        if not any(str(getattr(v, "vehicle_type", "")).strip().lower() in allowed_types for v in current_vehicles):
            return False, "Gate 3: Commercial relocation vehicle required for Packers & Movers."

    # GT audit Update 14 fix: fine-grained purchased-tier compatibility.
    #
    # Reads the purchased tier's classification from the quote-time
    # snapshot (ServiceRequest.fare_breakdown -- see Customer/backend/
    # service_requests/services/logistics_pricing.py's
    # quote_logistics_fare() for goods_transport_*, and
    # packers_movers_pricing.py's "vehicle"."payload_kg" for
    # packers_movers), never the mutable ServiceTier row -- a later
    # catalog edit must not retroactively change what an already-placed
    # booking requires.
    #
    # FAILS CLOSED when the snapshot is present but the value is
    # unrecognized/invalid, or when no active vehicle meets it -- no
    # candidate is allowed through by guessing. When the snapshot key is
    # entirely ABSENT (a booking placed before this fix shipped, so it
    # was never captured), this check is skipped and the coarse
    # category check above is the only one applied to that booking --
    # deliberately, so this fix does not retroactively strand
    # already-placed bookings with no history of this data. This is a
    # judgment call, not something the source data could settle either
    # way; flagged here for visibility.
    fb = getattr(job, "fare_breakdown", None) if job is not None else None
    fb = fb if isinstance(fb, dict) else {}

    if service_name_clean == "packers_movers":
        required_payload_kg = (fb.get("vehicle") or {}).get("payload_kg") if isinstance(fb.get("vehicle"), dict) else None
        if required_payload_kg not in (None, "", 0, "0", "0.0"):
            try:
                required_payload_kg = Decimal(str(required_payload_kg))
            except Exception:
                logger.debug(f"[9GATE_REJECT_GATE3_CAPACITY_UNRECOGNIZED] Job's purchased payload_kg '{required_payload_kg}' is not a valid number.")
                return False, "Gate 3: Purchased relocation vehicle's required payload could not be determined."
            capacities = [v.capacity_kg for v in current_vehicles if v.capacity_kg is not None]
            best_capacity = max(capacities) if capacities else None
            if best_capacity is None or best_capacity < required_payload_kg:
                logger.debug(f"[9GATE_REJECT_GATE3_CAPACITY_INSUFFICIENT] Employee #{emp.id}'s best vehicle capacity {best_capacity} < required {required_payload_kg}kg.")
                return False, f"Gate 3: No active vehicle meets the purchased relocation's required payload of {required_payload_kg}kg."

    elif service_name_clean in (
        "goods_transport_truck",
        "goods_transport_two_wheeler",
        # GT audit Update 16: the bare "goods_transport" slug is a live
        # category (it is in LOGISTICS_SERVICE_CATEGORIES above and the
        # Customer app emits it), but it has NO coarse allowed_types
        # branch above -- the slug does not say truck or two-wheeler, so
        # one could not be written. That left it with vehicle-presence
        # and document-currency checks only: a scooter could be
        # dispatched to a booking whose purchased tier was a truck.
        # The purchased tier's class is the one authority that DOES know
        # which it is, so the fine-grained check below is the correct
        # (and only) place to enforce it. It still fires only when a
        # quote-time snapshot is present, so nothing is over-restricted.
        "goods_transport",
    ):
        purchased_class = str(fb.get("vehicle_class") or "").strip().lower()
        if purchased_class:
            required_rank = _TIER_VEHICLE_CLASS_RANK.get(purchased_class)
            if required_rank is None:
                logger.debug(f"[9GATE_REJECT_GATE3_CLASS_UNRECOGNIZED] Job's purchased vehicle_class '{purchased_class}' is not recognized.")
                return False, f"Gate 3: Purchased vehicle class '{purchased_class}' is not recognized."
            ranks = [
                _EMPLOYEE_VEHICLE_TYPE_RANK.get(str(getattr(v, "vehicle_type", "")).strip().lower())
                for v in current_vehicles
            ]
            ranks = [r for r in ranks if r is not None]
            best_rank = max(ranks) if ranks else None
            if best_rank is None or best_rank < required_rank:
                logger.debug(f"[9GATE_REJECT_GATE3_CLASS_INCOMPATIBLE] Employee #{emp.id}'s best vehicle rank {best_rank} < required {required_rank} ('{purchased_class}').")
                return False, f"Gate 3: No active vehicle is class-compatible with the purchased '{purchased_class}' tier."

    return True, ""


def check_candidate_eligibility(emp: Employee, service_name: Optional[str] = None, job=None, check_workload: bool = True) -> Tuple[bool, str, Dict[str, bool]]:
    """
    ...
    if service_name_clean in LOGISTICS_SERVICE_CATEGORIES:
        # Update 18: the logistics compatibility rules that used to live
        # inline here now have a single definition in
        # check_purchased_vehicle_compatibility() above, so offer acceptance
        # enforces the identical rule without re-running the other nine
        # gates. Behaviour here is unchanged -- same checks, same order,
        # same messages.
        compatible, compat_reason = check_purchased_vehicle_compatibility(
            emp, service_name_clean, job
        )
        if not compatible:
            gate_results["G3"] = False
            return False, compat_reason, gate_results
    ...
    if check_workload and emp:
        ...
        if active_job:
            ...
            req_id = getattr(active_job, "request_id", None) or getattr(active_job, "id", None)
            return False, f"Gate 9: Technician is busy on active Job #{active_job.id} ({req_id}).", gate_results
...
def get_eligible_candidates(job_id_or_obj, max_gps_age_seconds: int = MAX_GPS_AGE_SECONDS, exclude_employee_ids: Optional[List[int]] = None, radius_km: float = MAX_DISPATCH_RADIUS_KM) -> List[Dict[str, Any]]:
    ...
    for emp in candidates_qs:
        ...
        if emp.id in _employees_holding_offers:
            logger.info(f"[DISPATCH_REJECT] employee={emp.id} reason=ALREADY_HAS_LIVE_OFFER")
            continue
...
def dispatch_job(job_id_or_obj, max_gps_age_seconds: int = MAX_GPS_AGE_SECONDS, exclude_employee_ids: Optional[Any] = None) -> Tuple[bool, str]:
    """
    Authoritative dispatch entry point. Evaluates geo-proximity, compliance,
    and skills to offer the job to the single best available candidate.
    """
...
def _dispatch_job_locked(job_id, max_gps_age_seconds: int = MAX_GPS_AGE_SECONDS, exclude_employee_ids: Optional[Any] = None) -> Tuple[bool, str]:
    from workforce_api.models import WorkforceEventLog
    ...
    with transaction.atomic():
        ...
        # Date-based safety gate:
        # 1. Past preferred_date cannot be dispatched
        # 2. Past created_at (for immediate bookings where preferred_date is None) cannot be dispatched
        pref_date = getattr(job_obj, "preferred_date", None)
        if pref_date and pref_date < today:
            logger.info(f"[DISPATCH_REJECT] job={job_obj.id} reason=SCHEDULE_DATE_EXPIRED (pref_date={pref_date} < today={today})")
        ...
        if not pref_date:
            ...
            if created_dt:
                ...
                if created_date < today:
                    logger.info(f"[DISPATCH_REJECT] job={job_obj.id} reason=SCHEDULE_DATE_EXPIRED (created_at={created_date} < today={today})")
                    return False, "SCHEDULE_DATE_EXPIRED"

        # Booking Dispatch Framework section 2: Scheduled Bookings
        is_future, scheduled_dt, window_open = get_scheduled_dispatch_window(job_obj, now=now)
        if is_future:
            scheduled_str = scheduled_dt.strftime("%Y-%m-%d %H:%M:%S") if scheduled_dt else "future"
            window_str = window_open.strftime("%Y-%m-%d %H:%M:%S") if window_open else "lead-window"
            logger.info(
                f"[DISPATCH_HOLD] job={job_obj.id} scheduled_start={scheduled_str} "
                f"dispatch_window_opens={window_str} reason=FUTURE_SCHEDULED_OUTSIDE_WINDOW"
            )
            return False, f"Job #{job_id} is scheduled for {scheduled_str}. Dispatch window opens at {window_str}."
        ...
        # Walk ranked candidates with row locking
        top_candidate = None
        top_emp = None
        top_dist_km = None
        top_score = None
...
def expire_and_reassign_offers() -> int:
    ...
    for offer in expired_offers:
        ...
        # Check if the job itself is past-dated before re-dispatching
        job_obj = getattr(offer, "job", None)
        is_past_job = False
        if job_obj:
            pref_date = getattr(job_obj, "preferred_date", None)
            if pref_date and pref_date < today:
...
def dispatch_pending_jobs(company_id=None, limit: int = 50) -> Dict[str, Any]:
    ...
    now = timezone.now()
    today = timezone.localdate()
    date_q = Q(preferred_date=today) | (Q(preferred_date__isnull=True) & Q(created_at__date=today))
