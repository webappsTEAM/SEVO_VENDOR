...
class VendorEstimationConfirmView(APIView):
    ...
    def post(self, request, pk):
        ...
        # GT audit Update 17: ownership is derived from the AUTHENTICATED
        # actor, never self-reported in the request body.
        #
        # Before this, any authenticated vendor-side actor could claim an
        # unclaimed lead while recording an arbitrary vendor_id: attributing
        # the lead to another company, or parking it under a key nobody
        # holds so the legitimate vendor can never claim or see it again
        # (_may_access_estimation compares sr.vendor_id against the actor's
        # own key, so a bogus value locks everyone out permanently).
        #
        # The one caller that legitimately acts on another vendor's behalf
        # is a platform superuser, so the request-supplied override is kept
        # for exactly that case and for no one else. Everything else is
        # derived from the actor, using the SAME helper
        # _may_access_estimation reads back -- so an actor who claims a lead
        # can always recognise it afterwards.
        is_platform_admin = bool(getattr(request.user, "is_superuser", False))
        if is_platform_admin and request.data.get("vendor_id"):
            ...
            vendor_id = _actor_vendor_key(request)
            vendor_name = str(
