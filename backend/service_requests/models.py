...
class ServiceRequest(models.Model):
    ...
    # GT audit Update 14 fix: the shared table has carried this column since
    # before this mirror existed (Customer's service_requests/models.py,
    # ServiceRequest.fare_breakdown) -- it was simply never declared here.
    # No DB schema change, no new migration (managed=False), same
    # X-04/GT-D-02 pattern as the other mirror fields in this file. This is
    # the quote-time snapshot of what was actually purchased (tier_id,
    # rates used, and -- as of this fix -- the purchased tier's
    # vehicle_class/weight_class, see logistics_pricing.py
    # quote_logistics_fare()) -- immutable once the booking is created, so
    # it is the correct place for automatic_dispatch.py's eligibility gate
    # to read the purchased vehicle requirement from, instead of the
    # coarse service_category string it used before.
