...
export default function CustomerDecisionPanel({
  ...
  const handleOpenAdminReview = (action) => {
    setAdminReviewAction(action);
    setAdminNotes(action === 'SEND_BACK' ? 'Please adjust the labor cost and re-verify capacitor ratings.' : 'Approved based on on-site inspection findings.');
  ...
  return (
    <div className="space-y-4">
      ...
      {/* STATE 1: SUBMITTED FOR ADMIN REVIEW */}
        ...
        <div className="p-4 bg-indigo-50 border border-indigo-200 rounded-xl space-y-3">
          <div className="flex items-start justify-between gap-3">
              ...
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <h4 className="text-xs font-bold text-indigo-950">
                    Quotation #{quote?.quote_ref || ''} Submitted for Customer Admin Review
                  </h4>
                ...
                <p className="text-xs text-indigo-800 leading-relaxed">
                  Quotation total: <strong>₹{Number(quote?.total_amount || 0).toLocaleString('en-IN')}</strong>. Per policy, Customer Admin reviews rates before quotation is released to {estimation?.customer_name}.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                type="button"
                onClick={() => handleOpenAdminReview('APPROVE')}
                className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-lg shadow-xs flex items-center gap-1.5 transition-colors"
              >
              ...
              <button
                type="button"
                onClick={() => handleOpenAdminReview('SEND_BACK')}
                className="px-2.5 py-1.5 bg-white border border-indigo-300 hover:bg-indigo-50 text-indigo-800 font-bold text-xs rounded-lg shadow-xs flex items-center gap-1.5 transition-colors"
              >
          ...
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl border border-zinc-200 p-6 space-y-5">
            ...
            <div className="flex items-center justify-end gap-2 pt-3 border-t border-zinc-100">
              ...
              >
                ...
                <span>{adminReviewAction === 'APPROVE' ? 'Approve & Release to Customer' : 'Send Back to Technician'}</span>
              </button>
            </div>
          </div>
        </div>
